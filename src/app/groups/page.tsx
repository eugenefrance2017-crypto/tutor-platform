...
import { Users, UserPlus, Trash2, Edit, CreditCard, X, Sparkles, Crown, Zap } from "lucide-react";
...
// ✅ Функция отправки Telegram (используем твой API роут)
...
function GroupsContent() {
  ...
  // ✅ ИСПРАВЛЕНО: status: "active" заменено на active: true, чтобы дашборд видел группу
  const saveGroup = async (e: React.FormEvent) => {
    ...
    const groupData = {
      ...
      active: true, // <-- ВОТ ЭТО ИСПРАВЛЕНИЕ
  ...
  // ✅ НОВАЯ ФУНКЦИЯ: Автоматическое создание счетов для группы
  const generateInvoices = async (group: any) => {
    ...
    const amount = group.price_per_lesson * 4; // Сумма за 4 занятия
    const deadline = new Date();
    deadline.setDate(deadline.getDate() + 5); // Дедлайн через 5 дней
    ...
    for (const studentId of group.student_ids) {
      try {
        ...
        successCount++;
        // Отправляем уведомление (не блокируя цикл)
        sendTelegramNotification(studentId, notificationMsg);
      } catch (e) {
    ...
    <div className={`min-h-screen relative transition-colors duration-500 ${isDark ? 'bg-gradient-to-br from-purple-950 via-fuchsia-950 to-slate-950' : 'bg-gradient-to-br from-purple-50 via-fuchsia-50 to-indigo-50'}`}>
      ...
      <div className="max-w-6xl mx-auto p-4 sm:p-6 pt-20 relative z-10">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
          <div>
            <h1 className={`text-3xl sm:text-4xl font-serif font-bold bg-gradient-to-r ${isDark ? 'from-purple-300 via-fuchsia-300 to-amber-200' : 'from-purple-700 via-fuchsia-600 to-purple-800'} bg-clip-text text-transparent`}>
              👑 Группы
            </h1>
          ...
          <motion.button 
            ...
            className="flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-purple-600 to-fuchsia-600 text-white rounded-xl font-bold shadow-lg shadow-purple-500/30 hover:shadow-fuchsia-500/40 transition"
          >
        ...
        {groups.length === 0 ? (
          ...
          >
            ...
            <p className={`text-sm mt-1 ${isDark ? 'text-purple-400/60' : 'text-purple-500'}`}>Нажмите "Создать группу", чтобы начать своё путешествие</p>
          </motion.div>
        ) : (
            ...
            {groups.map((group, idx) => {
              ...
              return (
                <motion.div 
                  ...
                  className={`group relative rounded-2xl p-5 border backdrop-blur-sm shadow-sm hover:shadow-xl hover:shadow-purple-500/10 transition-all duration-300 ${isDark ? 'bg-purple-900/40 border-purple-500/30 hover:border-fuchsia-400/50' : 'bg-white/80 border-purple-100 hover:border-fuchsia-300'}`}
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl shadow-inner ${group.subject === 'chemistry' ? 'bg-fuchsia-500/20 text-fuchsia-300' : 'bg-purple-500/20 text-purple-300'}`}>
                  ...
                  <div className="space-y-4 mb-5">
                    <div>
                      <div className="flex justify-between text-xs mb-1.5">
                        <span className={`font-serif italic ${isDark ? 'text-purple-300/70' : 'text-purple-600'}`}>Заполненность</span>
                        <span className={`font-bold ${fillPercent >= 100 ? 'text-amber-400' : 'text-fuchsia-400'}`}>{studentCount} / {group.max_students}</span>
                      </div>
                      <div className={`w-full h-2.5 rounded-full overflow-hidden ${isDark ? 'bg-purple-950' : 'bg-purple-100'}`}>
                        <motion.div 
                          initial={{ width: 0 }}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div className={`p-3 rounded-xl border ${isDark ? 'bg-purple-950/50 border-amber-500/20' : 'bg-amber-50/50 border-amber-100'}`}>
                        <p className="text-[10px] uppercase tracking-wider font-bold text-amber-500/80 mb-1 flex items-center gap-1"><Crown size={10} /> Доход / мес</p>
                        <p className="text-lg font-serif font-bold text-amber-400">~{monthlyIncome.toLocaleString()} ₽</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <motion.button 
                      ...
                      className="flex items-center justify-center gap-1.5 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-bold text-xs hover:shadow-lg hover:shadow-emerald-500/20 transition"
                    >
                      <Zap size={14} /> Счета (×4)
                    </motion.button>
                    <motion.button 
                      ...
                      className="flex items-center justify-center gap-1.5 py-2.5 bg-gradient-to-r from-amber-500 to-yellow-500 text-purple-950 rounded-xl font-bold text-xs hover:shadow-lg hover:shadow-amber-500/20 transition"
                    >
              ...
              <form onSubmit={saveGroup} className="p-6 space-y-5">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="number" value={formMaxStudents} onChange={(e) => setFormMaxStudents(parseInt(e.target.value) || 1)} min={1} max={20} className={`w-full border rounded-xl p-3 text-sm font-medium outline-none focus:ring-2 focus:ring-fuchsia-500/50 ${isDark ? 'bg-purple-950/50 border-purple-500/30 text-white focus:border-fuchsia-400' : 'bg-purple-50/50 border-purple-200 text-purple-900 focus:border-fuchsia-500'}`} />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className={`border rounded-xl p-3 max-h-40 overflow-y-auto space-y-1.5 custom-scrollbar ${isDark ? 'bg-purple-950/50 border-purple-500/30' : 'bg-purple-50/50 border-purple-200'}`}>
                ...
                <div className="flex gap-3 pt-2">
                  <motion.button 
                    ...
                    whileTap={{ scale: 0.98 }}
                    type="submit" 
                    className="flex-1 py-3 bg-gradient-to-r from-purple-600 to-fuchsia-600 text-white rounded-xl font-bold hover:shadow-lg hover:shadow-fuchsia-500/30 transition"
                  >
