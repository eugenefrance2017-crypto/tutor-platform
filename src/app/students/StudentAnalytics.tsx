// ФАЙЛ: app/students/StudentAnalytics.tsx
// ПРАВКИ ПРИМЕНЕНЫ:
// 1. homeworks / submissions / lessons теперь приходят пропсами из
//    page.tsx (там уже есть живые подписки на них) — убраны дублирующие
//    onSnapshot на "student_results" и на всю коллекцию "lessons".
//    Раньше "lessons" читался ЦЕЛИКОМ (без where) и фильтровался на
//    клиенте — дорого по чтениям и давало другие числа, чем вкладка
//    "Статистика". Теперь оба места используют один и тот же массив.
// 2. Исправлен баг: avgScore делился на 10 лишний раз (allScores уже
//    в шкале 0-100, деление в 10 раз занижало результат).
// 3. trialStats пересчитан из homeworks (type: 'trial_exam') +
//    submissions — как и вкладка "Пробники" в page.tsx — вместо
//    устаревшей/несинхронной коллекции student_results.
// 4. lastScore считается отдельно от avgScore (раньше в блоке
//    "Прогноз ЕГЭ" оба поля "Последний" и "Средний" показывали одно
//    и то же число).
// 5. dailyAnswers/dailyTasks/profile (геймификация, XP, серии) — это
//    отдельная система, не связанная с обычными ДЗ репетитора,
//    оставлена как есть, с собственными подписками.
